...
public class JSONParser {
	 ...
	 * 
	 * <p>
	 * This method has been deprecated. Please call either {@link #parseStrict(String)} (for inputs that strictly follow the JSON specification) or {@link #parseLenient(String)}. The implementation of this method calls parseLenient.
	 * 
	 * @param jsonString
	 ...
	 * 
	 * @deprecated use {@link #parseStrict(String)} or {@link #parseLenient(String)}
	 */
	@Deprecated
	 ...
	 * input is validated as described in RFC 4627 for safety and passed to {@code eval()}.
	 * 
	 * @param jsonString
	 *           a JSON object to parse
	 * @return a JSONValue that has been built by parsing the JSON string
	...
	public static JSONValue parseStrict(String jsonString) {
		return convertFromOrgJson(new org.json.JSONObject(jsonString));
		// return parse(jsonString, true);
	}
	
	private static JSONObject convertFromOrgJson(org.json.JSONObject orgObject) {
		JSONObject result = new JSONObject();
		for (String key : orgObject.keySet()) {
			Object value = orgObject.get(key);
			if (value instanceof org.json.JSONObject) {
				result.put(key, convertFromOrgJson((org.json.JSONObject) value));
			} else {
				// the GWT json parser parses other primitives as strings
	...
	/**
	 * Called from {@link #initTypeMap()}. This method returns a <code>null</code> pointer, representing JavaScript <code>undefined</code>.
	 */
